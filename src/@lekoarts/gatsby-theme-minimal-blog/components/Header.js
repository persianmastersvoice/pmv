...
const Header = () => {
  ...
  return (
    <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px' }}>
      <Link to="/">
        <img src="/logo.png" alt="Logo" style={{ height: '80px', width: 'auto', maxWidth: '100%' }} />
      </Link>
      ...
      <nav style={{ display: 'none', '@media (min-width: 500px)': { display: 'flex' }, opacity: 0.4 }}>
        ...
        {isOpen && (
          <nav style={{ position: 'absolute', top: '80px', left: 0, right: 0, backgroundColor: '#fff', padding: '16px' }}>
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '16px', margin: 0 }}>
              <li>
                <Link to="/blog">Blog</Link>
              </li>
              <li>
